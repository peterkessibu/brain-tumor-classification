...
try:
    ...
    st.error(f"Failed to initialize Generative AI model: {e}")
    st.stop()
...
def generate_saliency_map(model, img_array, class_index, img_size, input_file, is_upload=True):
    try:
        ...
        original_img = image.img_to_array(image.load_img(input_file, target_size=img_size))
...
if uploaded_file is not None or st.session_state.selected_image is not None:
    ...
    if selected_model == "Transfer Learning - Xception":
        classification_model = load_xception_model('xception_model.weights.h5')
        img_size = (299, 299)
    ...
    try:
        ...
        st.error(f"Error determining prediction: {e}")
        st.stop()
    ...
    saliency_map_path = generate_saliency_map(
        ...
        class_index, 
        img_size, 
        uploaded_file if uploaded_file is not None else selected_sample,
